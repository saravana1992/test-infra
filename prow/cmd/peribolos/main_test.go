...
func (c fakeDumpClient) ListCollaborators(org, repo string, onlyOutsideCollaborators bool) ([]github.User, error) {
	if org != c.name {
		return nil, fmt.Errorf("bad org: %s", org)
	}
	...
	members := make([]github.User, len(c.members))
	for _, u := range c.members {
		members = append(members, github.User{Login: u})
	}
