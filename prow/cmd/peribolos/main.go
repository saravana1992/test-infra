...
func (o *options) parseArgs(flags *flag.FlagSet, args []string) error {
	...
	flags.BoolVar(&o.fixRepoCollaborators, "fix-repo-collaborators", false, "Add/remove repo collaborators if set")
...
type dumpClient interface {
	...
	ListCollaborators(org, repo string, onlyOutsideCollaborators bool) ([]github.User, error)
}
...
func dumpOrgConfig(client dumpClient, orgName, repos string, dumpRepos bool) (*org.Config, error) {
	...
	if dumpRepos {
		out.Repos, err = createRepoConfig(client, orgName, repos, out.Members)
		if err != nil {
...
func createRepoConfig(client dumpClient, orgName, repoList string, members []string) (map[string]org.Repo, error) {
	...
	for _, repo := range repos {
		...
		oc, err := client.ListCollaborators(orgName, repo, true)
		if err != nil {
			return nil, err
		}
		var ocAdmin, ocWrite, ocRead []string
		for _, user := range oc {
			permission, err := client.GetUserPermission(orgName, repo, user.Login)
			if err != nil {
				return nil, err
			}
			switch permission {
			case string(org.Admin):
				ocAdmin = append(ocAdmin, user.Login)
			case string(org.Write):
				ocWrite = append(ocWrite, user.Login)
			case string(org.Read):
				ocRead = append(ocRead, user.Login)
			}
		}

		// get members with admin and write access
		// don't consider read access for now because all members have implicit read access
		var membersAdmin, membersWrite []string
...
func configureRepoCollaborators(opt options, client repoClient, orgName string, orgConfig org.Config, invitees sets.String) error {
	...
	for _, repo := range repos {
		...
		if err := configureUserCollaborators(opt, client, orgName, repo, config.Members, invitees, members, false); err != nil {
			errs = append(errs, err)
			continue
		}
		if err := configureUserCollaborators(opt, client, orgName, repo, config.OutsideCollaborators, invitees, members, true); err != nil {
			errs = append(errs, err)
			continue
		}
...
func configureUserCollaborators(opt options, client repoClient, orgName, repo string, config org.UserPermissionLevel, invitees, members sets.String, isOutsideCollaborator bool) error {
	// get desired state
	...
	if isOutsideCollaborator {
		outsideCollaborators, err := client.ListCollaborators(orgName, repo, true)
		if err != nil {
			return err
		}
	...
	for _, user := range users {
		...
		case string(org.Read):
			// don't consider read for members for now since all of them have implicit read access
			if isOutsideCollaborator {
