...
func TestCherryPickUnapprovedLabel(t *testing.T) {
	var testcases = []struct {
		...
		{
			name:          "not a release branch -> no-op",
			branch:        "master",
		...
		{
			name:          "same as above, but PR reopened -> add cpUnapproved and comment",
			branch:        "release-1.10",
	...
	for _, tc := range testcases {
		...
		err := handlePR(fc, logrus.WithField("plugin", "fake-cherrypick-unapproved"), &event)
		switch {
		case err != nil:
