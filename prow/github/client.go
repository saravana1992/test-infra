...
//
// If onlyOutsideCollaborators is true, it returns a list of only outside
// collaborators who have access to the repo.
//
// See 'IsCollaborator' for more details.
// See https://developer.github.com/v3/repos/collaborators/
func (c *Client) ListCollaborators(org, repo string, onlyOutsideCollaborators bool) ([]User, error) {
	...
	if onlyOutsideCollaborators {
		path = fmt.Sprintf("/repos/%s/%s/collaborators?affiliation=outside", org, repo)
	}
	var users []User
	err := c.readPaginatedResults(
		path,
